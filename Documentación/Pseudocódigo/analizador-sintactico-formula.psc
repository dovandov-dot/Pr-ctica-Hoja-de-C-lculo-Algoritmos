...
Funcion resultado <- operarPorcentaje(tokensFormula Por Referencia, pocisionToken Por Referencia, matrizAST Por Referencia, ultimoNodoCreado Por Referencia, filas)
	...
	numeroIzquierdo <- operarNunmeroFuncionCeldaParéntesisRango(tokensFormula, pocisionToken, matrizAST, ultimoNodoCreado)
	...
	Mientras pocisionToken <= filas Y tokensFormula[pocisionToken, 2] = "%"  Hacer
		...
		Si pocisionToken+ 1 <= filas Y tokensFormula[pocisionToken + 1, 1] = "Numero" Entonces
			...
			numeroDerecho <- operarNunmeroFuncionCeldaParéntesisRango(tokensFormula, pocisionToken,matrizAST, ultimoNodoCreado)
			//Creamos la primera fila del arbol AST
			ultimoNodoCreado <- ultimoNodoCreado +1 
			//Ingresamos todos los datos de los nodos de cada fila
			matrizAST[ultimoNodoCreado,1] <- "Operador"
...
Funcion valorLadoIzquierdoDerecho <- operarNunmeroFuncionCeldaParéntesisRango (tokensFormula Por Referencia, pocisionToken Por Referencia, matrizAST Por Referencia, ultimoNodoCreado Por Referencia)
...
Algoritmo Analizador_Sintáctico_Fórmula
	...
	Dimension textoSeparado[100,2] //Esta parte de código es para ejemplificar la matrix donde tenemos la formula ya tokenizada
	resultadoFinal <- 0
	pocisionToken <- 1
	ultimoNodoCreado <- 0
	i <- 1
	
	textoSeparado[1,1] = "Numero" //Esta parte de código es para ejemplificar el array donde tenemos la formula ya tokenizada
    textoSeparado[1,2] = "5" //Esta parte de código es para ejemplificar el array donde tenemos la formula ya tokenizada
    textoSeparado[2,1] = "Operador" //Esta parte de código es para ejemplificar el array donde tenemos la formula ya tokenizada
    textoSeparado[2,2] = "+" //Esta parte de código es para ejemplificar el array donde tenemos la formula ya tokenizada
	textoSeparado[3,1] = "Numero" //Esta parte de código es para ejemplificar el array donde tenemos la formula ya tokenizada
    textoSeparado[3,2] = "3" //Esta parte de código es para ejemplificar el array donde tenemos la formula ya tokenizada
	textoSeparado[4,1] = "Operador" //Esta parte de código es para ejemplificar el array donde tenemos la formula ya tokenizada
    textoSeparado[4,2] = "%" //Esta parte de código es para ejemplificar el array donde tenemos la formula ya tokeniza
	textoSeparado[5,1] = "Numero" //Esta parte de código es para ejemplificar el array donde tenemos la formula ya tokenizada
    textoSeparado[5,2] = "2" //Esta parte de código es para ejemplificar el array donde tenemos la formula ya tokenizadada
	contadorFilas <- 5 //Es para ejemplificar el dato que me dara el tokenizador
...
//	//Las columnas son tipo, valor, numeroIzquierdo, numeroDerecho
	...
	resultadoFinal <- operarSumaResta(textoSeparado, pocisionToken, matrizAST, ultimoNodoCreado, contadorFilas)	
